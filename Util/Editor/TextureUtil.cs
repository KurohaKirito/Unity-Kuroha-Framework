...
namespace Script.Effect.Editor.AssetTool.Util.Editor {
    ...
    public static class TextureUtil {
        ...
        public static bool IsSolidColor(Texture texture) {
            ...
            while (samplePoints.Count > 0 && sampleCountMax > 0 && iterateMax-- > 0) {
                ...
                for (var i = 0; i < LOOP; i++) {
                    ...
                    if (candidate.x >= 0 && candidate.x <= renderTextureWidth && candidate.y >= 0 && candidate.y <= renderTextureHeight) {
                        found = true;
        ...
        public static void GetTexturesInMaterial(Material material, out List<TextureData> textureDataList) {
            ...
            var depends = EditorUtility.CollectDependencies(new UnityEngine.Object[] {
                material
            });
            ...
            foreach (var depend in depends) {
                if (depend is Texture asset) {
                    ...
                    textureDataList.Add(new TextureData {
                        asset = asset, path = path, guid = guid
                    });
                }
            }
        }
