...
namespace Script.Effect.Editor.AssetTool.GUI.Editor
{
    public class SearchField
    {
        ...
        public bool OnGUI(ref string text)
        {
            if (textFieldRoundEdge == null)
            {
                ...
                transparentTextField = new GUIStyle(EditorStyles.whiteLabel) { normal = { textColor = EditorStyles.textField.normal.textColor } };
            }
            ...
            if (Event.current.type == EventType.Repaint)
            {
                ...
                localRoundEdgeStyle.Draw(position, string.IsNullOrEmpty(text)? new GUIContent("Input Search Text") : new GUIContent(""), 0);
