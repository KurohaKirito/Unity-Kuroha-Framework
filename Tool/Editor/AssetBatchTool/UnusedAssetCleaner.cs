...
namespace Script.Effect.Editor.AssetTool.Tool.Editor.AssetBatchTool {
    public static class UnusedAssetCleaner {
        ...
        public static void OnGUI() {
            ...
            unusedAssetFoldout = EditorGUILayout.Foldout(unusedAssetFoldout, AssetBatchToolGUI.batches[(int)AssetBatchToolGUI.BatchType.UnusedAssetChecker], true);
        ...
        public static List<ErrorInfo> Detect(UnusedAssetType detectType, string detectPath, bool isAutoCheck) {
            ...
            var guids = AssetDatabase.FindAssets(typeStr, new[] {
                detectPath
            });
            ...
            foreach (var key in references.Keys) {
                ...
                if (success == false && referenceCount <= 0) {
                    resultAuto.Add(new ErrorInfo {
                        type = ErrorType.NoneReference, assetPath = assetPath
                    });
                ...
                // 问题资源 2: 明明放在 "特定文件夹" 内, 但却有引用
                    ...
                    resultAuto.Add(new ErrorInfo {
                        type = ErrorType.HadReference, assetPath = assetPath
                    });
                    resultExport.Add($"{assetPath}\t\t被引用 {references[key].Count} 次");
                }
                // 正确资源
            ...
            if (isAutoCheck == false) {
                if (resultExport.Count > 0) {
                    ...
                    Dialog.Display($"检测结果位于: {outputPath} 文件中", Dialog.DialogType.Message, "OK");
                } else {
                    Dialog.Display("检测结束, 未检测到问题!", Dialog.DialogType.Message, "OK");
                }
            }
