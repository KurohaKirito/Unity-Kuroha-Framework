        ...
        private static void CheckSize(string assetPath, FileSystemInfo assetInfo, CheckItemInfo item, ref List<EffectCheckReportInfo> report) {
            ...
            if (ReferenceEquals(textureImporter, null) == false) {
                ...
                if (originWidth > width && originHeight > height) {
                    ...
                    if (textureImporter.GetPlatformTextureSettings("Android", out var maxSizeAndroid, out _)) {
                        ...
                        if (settings.maxTextureSize > width && settings.maxTextureSize > height) {
                            var asset = AssetDatabase.LoadAssetAtPath<Texture>(assetPath);
                            var content = $"未启用 Android 导入, 资源路径为: {assetInfo.FullName}, 纹理原始尺寸: ({originWidth}X{originHeight}), 当前 Default 导入设置: {settings.maxTextureSize} >>> 规范: ({width}X{height})";
                            report.Add(EffectCheckReport.AddReportInfo(asset, assetPath, EffectCheckReportInfo.EffectCheckReportType.TextureSize, content, item));
                        }
                    }
                    ...
                    if (textureImporter.GetPlatformTextureSettings("iPhone", out var maxSizeIPhone, out _)) {
                        ...
                        if (settings.maxTextureSize > width && settings.maxTextureSize > height) {
                            var asset = AssetDatabase.LoadAssetAtPath<Texture>(assetPath);
                            var content = $"未启用 iPhone 导入, 资源路径为: {assetInfo.FullName}, 纹理原始尺寸: ({originWidth}X{originHeight}), 当前 Default 导入设置: {settings.maxTextureSize} >>> 规范: ({width}X{height})";
                            report.Add(EffectCheckReport.AddReportInfo(asset, assetPath, EffectCheckReportInfo.EffectCheckReportType.TextureSize, content, item));
                        }
                    }
