        ...
        public static IEnumerable<string> GetMemoryDetail(ProfilerMemoryElement root, string filterName) {
            ...
            while (nodes.Count > 0) {
                ...
                // 筛选 2 级
                    ...
                    stage2 = currentText.IndexOf($"Texture2D{ProfilerMemoryElement.DELIMITER}", COMPARISON) >= 0 || currentText.IndexOf($"Mesh{ProfilerMemoryElement.DELIMITER}", COMPARISON) >= 0;
                    if (stage2 && stage1) {
                        texts.Add(currentText);
                    }
                }
