...
//			new ShaderTextureProperty("_DetailNormalMap",true),
        ...
        internal class TexturePipelineData
        {
            ...
            internal MeshBaker2PackingAlgorithmEnum _packingAlgorithm = MeshBaker2PackingAlgorithmEnum.UnityPackTextures;
        ...
        internal static IEnumerator __Step1_CollectDistinctMatTexturesAndUsedObjects(ProgressUpdateDelegate progressInfo,
                                                            ...
                                                            IMeshBaker2EditorMethods textureEditorMethods,
            ...
            for (int i = 0; i < data.allObjsToMesh.Count; i++)
            {
                ...
                { //for each submesh
                    ...
                    */
                            ...
                            if (txx != null)
                            {
                                if (txx is Texture2D)
                                {
                                    ...
                                    else
                                    {
                                        ...
                                        if (Application.isPlaying && data._packingAlgorithm != MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePackerFast)
                                        {
                    ...
                    MeshBaker2TextureTilingTreatment tilingTreatment = MeshBaker2TextureTilingTreatment.None;
                    if (data._fixOutOfBoundsUVs)
                    {
                        tilingTreatment = MeshBaker2TextureTilingTreatment.ConsiderUVs;
                    }
                                                                    ...
                                                                    MB2_LogLevel LOG_LEVEL)
        {
            ...
            if (data.distinctMaterialTextures.Count == 1 && data._fixOutOfBoundsUVs == false && data._considerNonTextureProperties == false)
            {
                ...
                data.distinctMaterialTextures[0].SetTilingTreatmentAndAdjustEncapsulatingSamplingRect(MeshBaker2TextureTilingTreatment.EdgeToEdgeXY);
            }
        ...
        internal static MB_ITextureCombinerPacker CreatePacker(bool onlyOneTextureInAtlasReuseTextures, MeshBaker2PackingAlgorithmEnum packingAlgorithm)
        {
            if (onlyOneTextureInAtlasReuseTextures)
            {
                return new MB3_TextureCombinerPackerOneTextureInAtlas();
            }
            else if (packingAlgorithm == MeshBaker2PackingAlgorithmEnum.UnityPackTextures)
            {
                return new MB3_TextureCombinerPackerUnity();
            }
            else if (packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePackerHorizontal)
            {
            ...
            else if (packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePackerVertical)
            {
                if (USE_EXPERIMENTAL_HOIZONTALVERTICAL)
                {
            ...
            else if (packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePacker)
            {
                return new MB3_TextureCombinerPackerMeshBaker();
            }
            else
        ...
        internal static IEnumerator __Step3_BuildAndSaveAtlasesAndStoreResults(MB3_TextureCombiner.CombineTexturesIntoAtlasesCoroutineResult result,
            ...
            IMeshBaker2EditorMethods textureEditorMethods, MB_AtlasesAndRects resultAtlasesAndRects,
        ...
        */
        ...
        internal static MB2_TexturePacker CreateTexturePacker(MeshBaker2PackingAlgorithmEnum _packingAlgorithm)
        {
            if (_packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePacker)
            {
                return new MB2_TexturePackerRegular();
            }
            else if (_packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePackerFast)
            {
                return new MB2_TexturePackerRegular();
            }
            else if (_packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePackerHorizontal)
            {
            ...
            else if (_packingAlgorithm == MeshBaker2PackingAlgorithmEnum.MeshBakerTexturePackerVertical)
            {
                MB2_TexturePackerHorizontalVert tp = new MB2_TexturePackerHorizontalVert();
