...
namespace DigitalOpus.MB.Core{
	...
	public class MB3_MeshBakerEditorInternal{
        ...
        void _init (SerializedObject mb) { 
			...
			combiner = meshBaker.FindProperty("meshCombiner");
		...
		public void DrawGUI(SerializedObject meshBaker, MB3_MeshBakerCommon target, System.Type editorWindowType){
            ...
            if (momm.MeshCombiner.lightmapOption == MeshBaker2LightmapOptions.PreserveCurrentLightmapping) {
			...
			if (momm.MeshCombiner.lightmapOption == MeshBaker2LightmapOptions.GenerateNewUV2Layout){
			...
			if (momm.MeshCombiner.outputOption == MeshBaker2OutputOptions.BakeIntoSceneObject){
			...
			} else if (momm.MeshCombiner.outputOption == MeshBaker2OutputOptions.BakeIntoPrefab){
			...
			} else if (momm.MeshCombiner.outputOption == MeshBaker2OutputOptions.BakeMeshAssetsInPlace){
		...
		public static bool bake(MB3_MeshBakerCommon mom){
			...
			bool success = false;
			try{
				if (mom.MeshCombiner.outputOption == MeshBaker2OutputOptions.BakeIntoSceneObject ||
                    mom.MeshCombiner.outputOption == MeshBaker2OutputOptions.BakeIntoPrefab)
                {
					...
					if (mom is MB3_MeshBaker){
						if (MB3_MeshCombiner.EVAL_VERSION){
							...
							if (!MB3_MeshBakerRoot.DoCombinedValidate(mom, MeshBakerObjectsToCombineTypes.PrefabOnly, new MB3_EditorMethods(), vl)) return false;
