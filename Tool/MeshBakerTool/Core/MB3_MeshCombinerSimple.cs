...
namespace DigitalOpus.MB.Core
{
    ...
    public class MB3_MeshCombinerSingle : MB3_MeshCombiner
    {
		...
		*/
        ...
        public class MeshChannelsCache
        {
            ...
            Vector2[] _getMeshUV2s(Mesh m)
            {
                ...
                if (uv.Length == 0)
                {
                    ...
                    if (this.mc._lightmapOption == MeshBaker2LightmapOptions.CopyUV2UnchangedToSeparateRects) Debug.LogError("Mesh " + m + " did not have a UV2 channel. Nothing to copy when trying to copy UV2 to separate rects. The combined mesh will not lightmap properly. Try using generate new uv2 layout.");
                    uv = new Vector2[m.vertexCount];
                    for (int i = 0; i < uv.Length; i++) { uv[i] = _HALF_UV; }
                }
                return uv;
        ...
        public override MeshBakerRenderType renderType
        {
            set
            {
                if (value == MeshBakerRenderType.SkinnedMeshRenderer && _renderType == MeshBakerRenderType.MeshRenderer)
                {
        ...
        [SerializeField]
        public List<GameObject> objectsInCombinedMesh = new List<GameObject>();
        ...
        {
            if (lightmapOption == MeshBaker2LightmapOptions.GenerateNewUV2Layout || lightmapOption == MeshBaker2LightmapOptions.PreserveCurrentLightmapping)
            {
        ...
        void _initialize(int numResultMats)
        {
            ...
            if (mbDynamicObjectsInCombinedMesh.Count > 0 &&
                ...
                renderType == MeshBakerRenderType.SkinnedMeshRenderer &&
        ...
        bool _addToCombined(GameObject[] goToAdd, int[] goToDelete, bool disableRendererInSource)
        {
            ...
            if (outputOption != MeshBaker2OutputOptions.BakeMeshAssetsInPlace &&
                renderType == MeshBakerRenderType.SkinnedMeshRenderer)
            {
            ...
            if (_doBlendShapes && renderType != MeshBakerRenderType.SkinnedMeshRenderer)
            {
            ...
            if (renderType == MeshBakerRenderType.SkinnedMeshRenderer && _goToDelete.Length > 0)
            {
            ...
            for (int i = 0; i < _goToDelete.Length; i++)
            {
                ...
                if (instance2Combined_MapTryGetValue(_goToDelete[i], out dgo))
                {
                    ...
                    if (renderType == MeshBakerRenderType.SkinnedMeshRenderer)
                    {
            ...
            for (int i = 0; i < _goToAdd.Length; i++)
            {
                ...
                if (!instance2Combined_MapContainsKey(_goToAdd[i].GetInstanceID()) || Array.FindIndex<int>(_goToDelete, o => o == _goToAdd[i].GetInstanceID()) != -1)
                {
                    ...
                    MeshBaker2TextureTilingTreatment[] tilingTreatment = new MeshBaker2TextureTilingTreatment[sharedMaterials.Length];
                    ...
                    if (_goToAdd[i] != null)
                    {
                        ...
                        if (lightmapOption == MeshBaker2LightmapOptions.PreserveCurrentLightmapping)
                        {
            ...
            for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
            {
                ...
                if (!dgo._beingDeleted)
                {
                    ...
                    if (renderType == MeshBakerRenderType.SkinnedMeshRenderer) { Array.Copy(boneWeights, dgo.vertIdx, nboneWeights, targVidx, dgo.numVerts); }
            ...
            if (renderType == MeshBakerRenderType.SkinnedMeshRenderer) boneWeights = nboneWeights;
            ...
            if (lightmapOption == MeshBaker2LightmapOptions.CopyUV2UnchangedToSeparateRects)
            {
        ...
        {
            bool doBones = false;
            if (renderType == MeshBakerRenderType.SkinnedMeshRenderer) doBones = true;
            Apply(true, true, _doNorm, _doTan, _doUV, doUV2(), _doUV3, _doUV4, doCol, doBones, doBlendShapes, uv2GenerationMethod);
        }

        public virtual void ApplyShowHide()
        {
            if (_validationLevel >= MeshBaker2ValidationLevel.Quick && !ValidateTargRendererAndMeshAndResultSceneObj()) return;
            if (_mesh != null)
            {
                if (renderType == MeshBakerRenderType.MeshRenderer)
                {
                          ...
                          GenerateUV2Delegate uv2GenerationMethod = null)
        {
            ...
            if (_mesh != null)
            {
                ...
                if (vertices)
                {
                    ...
                    if (verts.Length > 0) {
                        if (_recenterVertsToBoundsCenter && _renderType == MeshBakerRenderType.MeshRenderer)
                        {
                ...
                if (renderType != MeshBakerRenderType.SkinnedMeshRenderer && lightmapOption == MeshBaker2LightmapOptions.GenerateNewUV2Layout)
                {
                ...
                else if (renderType == MeshBakerRenderType.SkinnedMeshRenderer && lightmapOption == MeshBaker2LightmapOptions.GenerateNewUV2Layout)
                {
                    if (LOG_LEVEL >= MB2_LogLevel.warn) Debug.LogWarning("UV2 cannot be generated for SkinnedMeshRenderer objects.");
                }
                if (renderType != MeshBakerRenderType.SkinnedMeshRenderer && lightmapOption == MeshBaker2LightmapOptions.GenerateNewUV2Layout && do_generate_new_UV2_layout == false)
                {
                                        ...
                                        bool updateUV, bool updateUV2, bool updateUV3, bool updateUV4, bool updateColors, bool updateSkinningInfo, MeshChannelsCache meshChannelCache)
        {
            ...
            if (renderType == MeshBakerRenderType.SkinnedMeshRenderer && updateSkinningInfo)
            {
        ...
        public override bool AddDeleteGameObjectsByID(GameObject[] gos, int[] deleteGOinstanceIDs, bool disableRendererInSource)
        {
            ...
            if (validationLevel > MeshBaker2ValidationLevel.None)
            {
                ...
                if (gos != null)
                {
                    for (int i = 0; i < gos.Length; i++)
                    {
                        ...
                        if (validationLevel >= MeshBaker2ValidationLevel.Robust)
                        {
                ...
                if (deleteGOinstanceIDs != null && validationLevel >= MeshBaker2ValidationLevel.Robust)
                {
        ...
        public override void DestroyMeshEditor(IMeshBaker2EditorMethods editorMethods)
        {
            if (_mesh != null)
            {
                if (LOG_LEVEL >= MB2_LogLevel.debug) MB2_Log.LogDebug("Destroying Mesh");
        ...
        public bool ValidateTargRendererAndMeshAndResultSceneObj()
        {
            if (_resultSceneObject == null)
            {
                ...
                if (_targetRenderer == null)
                {
                    ...
                    if (_renderType == MeshBakerRenderType.SkinnedMeshRenderer)
                    {
                    ...
                    if (_renderType == MeshBakerRenderType.MeshRenderer)
                    {
        ...
        */
        internal static Renderer BuildSceneHierarchPreBake(MB3_MeshCombinerSingle mom, GameObject root, Mesh m, bool createNewChild = false, GameObject[] objsToBeAdded = null)
        {
            ...
            if (mom.renderType == MeshBakerRenderType.SkinnedMeshRenderer)
            {
        ...
        */
        ...
        private static void _ConfigureSceneHierarch(MB3_MeshCombinerSingle mom, GameObject root, MeshRenderer mr, MeshFilter mf, SkinnedMeshRenderer smr, Mesh m, GameObject[] objsToBeAdded = null)
        {
            ...
            if (mom.lightmapOption == MeshBaker2LightmapOptions.PreserveCurrentLightmapping || mom.lightmapOption == MeshBaker2LightmapOptions.GenerateNewUV2Layout)
            {
        ...
        public override void CheckIntegrity()
        {
            ...
            if (renderType == MeshBakerRenderType.SkinnedMeshRenderer)
            {
                ...
                for (int i = 0; i < mbDynamicObjectsInCombinedMesh.Count; i++)
                {
                    ...
                    if (renderType == MeshBakerRenderType.SkinnedMeshRenderer && dgo.indexesOfBonesUsed.Length < 1)
