...
namespace DigitalOpus.MB.Core
{
    ...
    public class MB_TexSet
    {
        ...
        private interface PipelineVariation{
            ...
            void SetTilingTreatmentAndAdjustEncapsulatingSamplingRect(MeshBaker2TextureTilingTreatment newTilingTreatment);
        ...
        private class PipelineVariationAllTexturesUseSameMatTiling : PipelineVariation
        {
                                                        ...
                                                        out Rect propsUseDifferntTiling_obUVRect)
            {
                ...
                if (texSet.tilingTreatment == MeshBaker2TextureTilingTreatment.EdgeToEdgeX)
                {
                    allPropsUseSameTiling_encapsulatingSamplingRect.x = 0;
                    allPropsUseSameTiling_encapsulatingSamplingRect.width = 1;
                }
                else if (texSet.tilingTreatment == MeshBaker2TextureTilingTreatment.EdgeToEdgeY)
                {
                    allPropsUseSameTiling_encapsulatingSamplingRect.y = 0;
                    allPropsUseSameTiling_encapsulatingSamplingRect.height = 1;
                }
                else if (texSet.tilingTreatment == MeshBaker2TextureTilingTreatment.EdgeToEdgeXY)
                {
                    allPropsUseSameTiling_encapsulatingSamplingRect = new Rect(0, 0, 1, 1);
                }
            }

            public void SetTilingTreatmentAndAdjustEncapsulatingSamplingRect(MeshBaker2TextureTilingTreatment newTilingTreatment)
            {
                Debug.Assert(texSet.allTexturesUseSameMatTiling == true);
                if (texSet.tilingTreatment == MeshBaker2TextureTilingTreatment.EdgeToEdgeX)
                {
        ...
        public MeshBaker2TextureTilingTreatment tilingTreatment { get; private set; }
        ...
        public MB_TexSet(MeshBakerMaterialTexture[] tss, Vector2 uvOffset, Vector2 uvScale, MeshBaker2TextureTilingTreatment treatment)
        {
            ts = tss;
