...
public abstract class MB3_MeshBakerRoot : MonoBehaviour {
	...
	public static bool DoCombinedValidate(MB3_MeshBakerRoot mom, MeshBakerObjectsToCombineTypes objectToCombineType, IMeshBaker2EditorMethods editorMethods, MeshBaker2ValidationLevel validationLevel){
		...
		for (int i = 0; i < objsToMesh.Count; i++){
			...
			if (m != null){ //This check can be very expensive and it only warns so only do this if we are in the editor.
				if (!Application.isEditor && 
					...
					validationLevel >= MeshBaker2ValidationLevel.Robust){
		...
		if (mom is MB3_MeshBaker)
		{
			...
			if (mom is MB3_MeshBaker && ((MB3_MeshBaker)mom).MeshCombiner.renderType == MeshBakerRenderType.SkinnedMeshRenderer){
		...
		if (editorMethods != null){
			editorMethods.CheckPrefabTypes(objectToCombineType, objsToMesh);
		}
		return true;
	}
}
