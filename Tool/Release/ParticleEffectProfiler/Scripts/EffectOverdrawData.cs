...
/// 处理像素计算
/// </summary>
public class EffectOverdrawData
{
    ...
    {
        var pixDrawAverage = GetPixDrawAverage();
        if (pixDrawAverage == 0)
        {
            return 0;
        }
    ...
    {
        return "特效原填充像素点：" + GetPixDrawAverage();
    }
    public string GetPixActualDrawAverageStr()
    {
        return "特效实际填充像素点：" + GetPixActualDrawAverage();
    }
    public string GetPixRateStr()
    {
        return "平均每像素overdraw率：" + GetPixRate();
    }
}
#endif